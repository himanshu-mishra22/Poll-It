    ...
    <div>
      {optionList.map((item, index) => (
        ...
        key={item.id}
         className="flex justify-between bg-gray-200/80 px-4 py-2 rounded-md mb-3">
          ...
          >
            <HiOutlineTrash className="cursor-pointer text-lg text-red-500" />
          </button>
        </div>
      ))}
      ...
      {optionList.length < 4 && (
        <div className="flex items-center gap-5 mt-4">
          ...
          />
          <button
            className=" cursor-pointer flex items-center gap-1 text-xs text-nowrap py-[6px]"
